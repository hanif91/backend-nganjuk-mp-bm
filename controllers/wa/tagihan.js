...
async function cekTagihanWa(req,res) {
	try {
...
async function infoPelanggan(req, res) {
  try {
		const { id,username,loket,nama } = req.auth;
		const isValiduser = await validateUserMitra(id);
		if (!isValiduser) {
    ...
    const resTagihanSdhLunas = await db.raw("call infobayar_deskinfo(?)", [nosamb]);
...
async function infoBayarWa(req,res) {
	try {
...
async function bayarTagihanWa(req,res) {
	try {
