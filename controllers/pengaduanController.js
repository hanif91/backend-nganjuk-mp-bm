...
async function createPengaduan(req, res) {
  try {
...
async function createPengaduanVercel(req, res) {
  try {
    const token = await req.auth.token_session;
    const session = await validateSessionToken(token);
    let user_id = null;
...
async function getJenisAduan(req, res) {
  try {
    const token = await req.auth.token_session;
    const session = await validateSessionToken(token);
    let user_id = null;
...
async function getAduan(req, res) {
  try {
    const token = await req.auth.token_session;
    const session = await validateSessionToken(token);
    if (session.session === null) {
    ...
    const Aduan = await db
      ...
      .leftJoin("pelanggan as c", "a.no_pelanggan", "c.nosamb")
      ...
      .where("user_web_id", session.user.id)
      .orderBy("tanggal", "asc")
      .limit(12);
    ...
    const dataOlahAduan = Aduan.map((val) => {
      ...
      const dataMasuk = {
        ...
        tanggal: val.tanggal,
        deskripsi: `Data aduan masuk ke server di input oleh ${session.user.nama}`,
      ...
      if (val.no_pelanggan != null) {
        nosambval = val.nosamb.trim();
        alamatval = val.alamat.trim();
        namaval = val.nama.trim();
      }
...
export { createPengaduan, getJenisAduan, getAduan, createPengaduanVercel };
